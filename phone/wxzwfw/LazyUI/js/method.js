...
!function(){
    var Lazy = function() {
        ...
        //设置头部
        this.setHeader = setHeaderFun;
        //隐藏头部
        this.setHeaderHide = setHeaderHideFun;
        this.setHeaderStop = setHeaderStopFun;
        //
        this.getCodeAction = getCodeFunAction;
    }
     /*///////////////////////////////////////////////////////////////////
    *   内置方法和变量可以直接使用
    ...
    */
   //
   function getCodeFunAction(str){
                var r = window.location.search.substr(1);
                var arr=r.split("&");
                var str="";
                for(var i=0;i<arr.length;i++){
                    if(arr[i].indexOf("code")!=-1)
                        str=arr[i].substring(arr[i].indexOf("=")+1);
                }
        return str;
    }
    ...
    function initFun (back) {
        ...
        switch(lazy.browserType) {
            ...
            break;
            case "weixin" :
                if(!lazy.geturlstr("wxinitId")||!lazy.getParameter("initid")) {
                	lazy.loadJs("weixinType", lazy.url+"LazyUI/js/weixin.js", function() {
                		commonFun();
                	});
                	return;
                };
                lazy.loadJs("weixinJs", "https://res.wx.qq.com/open/js/jweixin-1.0.0.js", function() {
                    lazy.loadJs("weixinType", lazy.url+"LazyUI/js/weixin.js", function() {
                        ...
                        lazy.URLRequest(ip+"/wechat/weixinevent/jsapi.ht?url="+url, function(data){
                        ...
                        /*
                        wx.ready(function(){
                        	if(!lazy.getParameter('wxconfig')) {
                        		lazy.setParameter('wxconfig',{
                                    appId: lazy.geturlstr("appId"), 
                                    timestamp: lazy.geturlstr("timestamp"), 
                                    nonceStr: lazy.geturlstr("nonceStr"), 
                                    signature: lazy.geturlstr("signature")
                                });
                        	}
                            commonFun();
                        });
    ...
    function POSTFun(url,successFun,errorFun,num) {
        if(num!=0) {
            var loading = loadingFun("", num);
        }
        var xmlhttp;
    ...
    function alert2Fun(obj) {
     ...
     function alert3Fun(titile,titile2, _obj) {
         //yestitle yesfun notitle nofun
        var obj = _obj?_obj:{};
        var alertDiv = lazy.creat("div","alertDiv3 ub ub-ver");
        var _html = '<div class="ub-f1"></div>';
        ...
        _html += '  <div class="alertbutton ub">';
        _html += '      <a class="alertbtn1 comcirm ub-f1"><span>'+(obj.yestitle?obj.yestitle:"知道了")+'</span></a>';
        _html += '  </div>';
        _html += '</div>';
        _html += '</div>';
        _html += '<div class="ub-f1"></div>';
   ...
   //弹出选择确定取消对话框
     function alertyesnoFun(title, yesFun, noFun,yestitle,notitle) {
         var aynDiv = document.getElementById("aynDiv");
         if(aynDiv==undefined) {
             aynDiv = document.createElement("div");
             aynDiv.id = "aynDiv";
             //15 * 7.5
             //if(lazyFontSize!=undefined)lazy.fontSize = lazyFontSize;
             var l = (document.body.clientWidth - lazy.fontSize*15)/2+"px";
             var t = (document.body.clientHeight - lazy.fontSize*7.5)/2+"px";
             aynDiv.innerHTML = '<div class="aynkuang" style="left:'+l+';top:'+t+';"><div class="ayntitle hh">'+title+'</div><div class="aynbtns ub"><div class="aynno">'+(notitle?notitle:"取消")+'</div><div class="ub-f1"></div><div class="aynyes">'+(yestitle?yestitle:"确定")+'</div></div></div>';
             document.body.appendChild(aynDiv); 
         }
         alert(aynDiv.id)
         aynDiv.style.display = "block";
         var aynno = aynDiv.getElementsByClassName("aynno")[0];
         var aynyes = aynDiv.getElementsByClassName("aynyes")[0];
         aynno.onclick = function() {
             aynDiv.style.display = "none";
             noFun();
         }
         aynyes.onclick = function() {
             aynDiv.style.display = "none";
             yesFun();
         }
    ...
    function getFlieTypeFun(houzhui) {
        if (houzhui == "png" || houzhui == "jpg" || houzhui == "jpeg" || houzhui == "gif" || houzhui == "PNG" || houzhui == "JPG" || houzhui == "JPEG" || houzhui == "GIF") {
            ...
            return "pdf";
        } else if(houzhui == "txt"||houzhui == "TXT"){
            return "txt";
        }
         else {
            return "file";
    ...
    //设置头部
    function setHeaderFun(obj) {
        //alert(lazy.browserType);
    }
    function setHeaderStopFun() {

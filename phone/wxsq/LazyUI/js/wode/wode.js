lazy.plugins.wode = {
    "init" : function(el,data) {
        ...
        function one(obj) {
            ...
            tr.innerHTML = _html; 
            if(obj.right){
                tr.getElementsByClassName("text")[0].style.backgroundImage="url("+obj.right+")"
            }else{
                tr.getElementsByClassName("text")[0].style.backgroundImage="url(../LazyUI/js/wode/right1.png)"
            }
            if(obj.onclick){
                tr.onclick = function() {
                    obj.onclick(obj);

        ...
        lazy.loadJsCss(["http://res.wx.qq.com/open/js/jweixin-1.0.0.js",lazy.url+"LazyUI/js/weixin.js"], function() {
            */
            ...
            if (document.location.href.indexOf("DDS") != -1) {
               ...
               lazy.GET(ip + "/wechat/weixinevent/jsapi.ht?url=" + url, function (data) {
                  //alert("http://vito.tunnel.qydev.com/xfwq/wechat/weixinevent/jsapi.ht?url=" + url);
                  //lazy.GET("http://vito.tunnel.qydev.com/xfzz/wechat/weixinevent/jsapi.ht?url=" + url, function (data) {
                  // alert(1)
                  //alert("22222http://vito.tunnel.qydev.com/lazy/getWeixin.php?url="+encodeURIComponent(document.location))
             ...
             wx.config({
                    ...
                    nonceStr: wxconfig.nonceStr, // 必填，生成签名的随机串
                    signature: wxconfig.signature,// 必填，签名，见附录1
                     jsApiList: ['chooseImage', 'previewImage', 'uploadImage','getLocation','startRecord','stopRecord','uploadVoice','playVoice','stopVoice','onVoicePlayEnd','downloadImage','onVoiceRecordEnd','closeWindow'] // 必填，需要使用的JS接口列表，所有JS接口列表见附录2

...
PAGE_INIT = function(parent) {
        ...
        setData([{
            ...
            contact : false,
                ...
                list : [{
                    ...
                    title : '热力图分析',
                    onclick : function() {
                        lazy.openWin('#lingdaotong/amap2?tablename=rlt_dispute_report_handled_hot&coding=7&time='+(mytime||config.data.time)+'&orgid='+org_id||'');
                    }
                }]
            },{
            id : 'gqycllpm_chart',
    ...
    function showChart(orgid, time) {
        ...
        var urls = [{
            key : 'table',
            chartid : 'gqycllpm_chart',
            url : urlConfig.gqycllpm + 'tablename=rlt_dispute_report_handled_rank&orgid=' + orgid +'&time='+time
        },{
            chartid : 'cllljtj_chart',
            url : urlConfig.cllljtj + 'tablename=rlt_dispute_report_handled_type&orgid=' + orgid + '&time=' + time+'&limit=10'
        }];
        var k = 0;
        function one() {
            lazy.URLRequestCache(urls[k].url, function(str) {
                ...
                if (urls[k].key && urls[k].key == 'table') {
                    var chartdata = [['排名', '地区', '数量']];
                    lazy.for(data.data || [], function(obj, i) {
                        chartdata.push([i + 1, obj.name, obj.value]);
                    })

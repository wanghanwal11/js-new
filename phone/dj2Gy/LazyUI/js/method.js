...
!function(){
    var Lazy = function() {
        ...
        this.alertlist = alertlistFun;
         this.alertlist1 = alertlistFun1;
         this.alerttxt1 = alerttxtFun1;
        //暂无数据
        ...
        this.dfsh = function(title,str){};
         //返回登录页
        this.siblings = siblingsFun;
        //
       // this.closeBounce = stopBounceFun;
    }
     /*///////////////////////////////////////////////////////////////////
    *   内置方法和变量可以直接使用
   ...
   //
    ...
    function initFun (back) {
        ...
        function commonFun() {
            lazy.loadJs("jq", lazy.url+"common/jquery-1.7.2.min.js", function() {
    ...
    function JSONPFun(url, back1, back2, num) {
        ...
        window[name] = function(data) {
            ...
            if(data.isSucceed) {
               ...
               if(back2)back2(data.message?data.message:"message is null");
               else alert2Fun(data.message?data.message:"message is null");
            }
        }
        var f = (url.indexOf('?')==-1?'?':'&');
        lazy.loadJs(name, url+f+'jsoncallback='+name, function() {});
         //jsoncallback
    }
    ...
    function bounceFun(obj) {
        ...
        if(el && lazyWindow.browserType=="ios") {
           ...
           bouncebutton.onclick = function() {
               document.body.appendChild(bouncebutton);
               if(obj.up && lazy.bounceBol)obj.up();
               lazy.bounceBol = false;
           }
        } else {
    ...
    //弹窗
    function alertFun(titile, _obj,back) {
    ...
    function alertlistFun(arr,back) {
     ...
     //弹出列表
    function alertlistFun1(arr,back) {
        //{"title" : "", onclick}
        var mask = lazy.creat("div", "alertlistmask ub ub-ver");
        mask.innerHTML = '<div class="ub-f1"></div><div class="alertlistdiv"><div class="alertlisttr"></div></div><div class="ub-f1"></div>'; 
        var alertlisttr = mask.getElementsByClassName("alertlisttr")[0];
        ...
        function one(obj) {
            var tr = lazy.creat("a", "tr");
            tr.innerHTML = '<span>'+obj.html+'</span>';
            tr.onclick = function() {
                if(obj.onclick)obj.onclick(obj);
                if(back) back(obj)
            }
     ...
     //弹出框
    function alerttxtFun1(obj,back) {
        //{"title" : "", onclick}
        var mask = lazy.creat("div", "alertlistmask ub ub-ver");
        mask.innerHTML = '<div class="ub-f1"></div><div class="alertlistdiv alertlistdiv1"><div class="alertlisttr alertlisttr1"></div></div><div class="ub-f1"></div>'; 
        var alertlisttr = mask.getElementsByClassName("alertlisttr")[0];
        var ubf1 = mask.getElementsByClassName("ub-f1");
        document.body.appendChild(mask);
        for(var i=0;i<ubf1.length;i++){
                ubf1[i].onclick = function() {
                document.body.removeChild(mask);
            }
        }
        ...
        function one(obj) {
            if(obj.type="textarea"){
                ...
                left.innerHTML=obj.left;
                var tr = lazy.creat("textarea", "txt");
                var right = lazy.creat("div", "right");
                right.innerHTML=obj.right;
                 alertlisttr.appendChild(left);
                 alertlisttr.appendChild(tr);
                 alertlisttr.appendChild(right);
                 right.onclick=function(){
                     if(back)back(tr.value)
                     document.body.removeChild(mask);
                 }
            }

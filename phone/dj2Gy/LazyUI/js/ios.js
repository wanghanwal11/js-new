...
lazy.openWin1 = function(url) {
    ...
    lazy.openWin("aj://presentNewWebView_?"+url.replace(/&/g,"-*-"))
}
//扫一扫
...
lazy.time=function(type,back){
    var funname = lazy.callBack(back);
    lazy.openWin("aj://dateNewPicker_?"+type+"&"+funname);
}
//缓存
lazy.setCache = function(str,back){
    var funname = lazy.callBack(back);
    lazy.openWin("aj://setAndCloseWebValue_?"+str+"&"+funname)
}

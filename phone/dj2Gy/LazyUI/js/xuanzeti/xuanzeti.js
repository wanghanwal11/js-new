lazy.plugins.xuanzeti = {
	"init" : function(el,data) {
		...
		//标题
		el.innerHTML = '<div class="ub titlename"><div class="t1">'+data.title+'</div><div class="ub-f1 t2"><span class="yhd">1</span>/'+arr[0].counter+'</div></div>';
		//
        ...
        if(arr[0].counter==1){
             buttondiv.innerHTML = '<span>完成</span>';
        }
        ...
        buttondiv.onclick = function() {
           ...
           if(data.getValue()[0].selectopoion==""){
               lazy.alert2("请选择答案")
               return;
           }
            b=true;
            ...
            //
            if(data.back)data.back(data.getValue(),p);
            if(p < arr.length-1) {
                bolc = true;
		...
		data.getValue = function() {
            ...
            //for(var i = 0; i < list.length; i++) {
                var id = list[p].getAttribute("trid");
                var value = list[p].getAttribute("value");
                dataarr.push({
                    "questionid" : id,
                    "selectopoion" : value,
                    "testanswer":arr[p].testanswer
                });
            //}
            return  dataarr;
		}
